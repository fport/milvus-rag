...
!!! done "Bu aşamanın sahibi olduğu yerler"
    ...
    `eval.py` (altın küme koşucusu), `evals/golden.example.jsonl` (şablon),
    `evals/results/<tarih>_<etiket>.json` (her koşu) ve README'deki ölçüm defteri.

## Tek kural

> Ölçülmemiş hiçbir şey yayınlanmaz. Her erişim değişikliği tabloya "daha iyi
...
eklendi ve sonra kapatıldı; rerank tabanlı çekimserlik kapısı en iyi negatif dedektörüydü
...
soru için de erişimci sekiz parça döndürüyor, en iyisi 0.366. Halüsinasyon tam olarak
orada başlıyor ve yalnızca cevaplanabilir sorulardan oluşan bir altın küme bunu göremez.
...
sayı tek başına anlamsız: her şeye çekimser kalan bir kapı, kusursuz bir abstain oranı
alır.

!!! measured "Hangi çekimserlik kapısı kaldı"

    | Kapı | abstain (negatifler) | false_weak (42 pozitif) | eklenen gecikme |
    |---|---|---|---|
    | yoğun < 0.55 notu | 10/12 = 0.833 | 2/42 = 0.048 | 0 |
...
Repoya özgü altın dosyalar gitignore'da. Yalnızca şablon yayınlanıyor: bir altın küme,
senin iç dosya yollarının listesidir.

## Kalibrasyon modelle birlikte gezer

Bu projedeki eşikler (`0.45` taban, `0.55` not) evrensel sabitler değil. **Bu korpusta
...
Tam tablo README'de duruyor; iki dilde tutuluyor, tarihli, korpus ve soru dağılımı
belirtilmiş hâlde. Burada kısaca:

!!! measured "Erişim modları — 318 dosyalık TypeScript monorepo, 42 soru, k=8"

    | Etiket | Ayar | Recall@8 | MRR | TR metin R@8 | p50 |
    |---|---|---|---|---|---|
    | bm25 | yalnızca BM25 | 0.405 | 0.240 | 0.263 | 2 ms |
    | hybrid | yoğun+BM25 → RRF | 0.786 | 0.604 | 0.684 | 40 ms |
    | dense | yalnızca yoğun | 0.786 | 0.678 | 0.684 | 32 ms |
    | **auto+dense** ✓ | sembol→bm25, metin→yoğun | **0.786** | **0.690** | 0.684 | 34 ms |
...
!!! measured "Parçalama — AST'ye karşı düz pencere, aynı korpus iki kez indekslendi"

    | Etiket | parça | Recall@8 | MRR | TR metin MRR | sembol MRR |
    |---|---|---|---|---|---|
    ...
    sorguları. AST parçalama daha fazlasını bulmuyor — doğru parçayı en üste koyuyor ve
    adını biliyor. Gramersiz bir dilin kaybettiği şeyin boyutu da bu: sıralama ve atıf,
    recall değil.

!!! measured "Zenginleştirme — 46 dosya / 423 parça, iki kez indekslendi"
    ...
    İngilizce dışı düz metinde +0.15 MRR, `false_weak` yarıya indi, çekimserlik
    ...
    olmaması: yerel bir 9B'de dakikada ~11 açıklama, 2.8k parçalık bir repo için ~4 saat
    demek. Sonrası artımlı, parça özeti + modele göre önbellekli.
...
2. **Beklentinin boyutunu belirliyor.** "AST parçalama +0.09 MRR eder, çoğu sembolde"
   kullanılabilir bir cümle. "AST parçalama daha iyidir" değil.
