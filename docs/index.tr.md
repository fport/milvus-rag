...
tree-sitter ile kod birimlerine bölünüyor, BGE-M3 (yoğun) ve BM25 (seyrek) ile Milvus'a
yazılıyor. Sorgu anında sembol biçimli bir sorgu BM25'e, düz bir cümle yoğun kanala
...
!!! measured "Burada ölçülmemiş hiçbir şey yayınlanmıyor"
    ...
    Her erişim kararının arkasında bir sayı var; negatif vakalar içeren bir altın küme
...
-   **[1. Kaynaklar ve senkronizasyon](01-sources.md)**
    ...
    Azure / GitHub / yerel, arkasında bir yoklayıcı olan push webhook'u, tek işçili bir
...
-   **[2. İndeksleme](02-indexing.md)**
    ...
    Gerçek sözdizim sınırlarında tree-sitter parçalama, hiçbir şey saklanmadan önce sır
    temizliği, BGE-M3 embedding'leri, yoğun ve BM25'in yan yana durduğu tek bir Milvus
    koleksiyonu.

    *Kapattığı tuzak:* sabit boyutlu pencere bir fonksiyonu ortasından keser; fazla uzun
    bir parçanın embedding'i ise hiçbir şeyi temsil etmeyen bir ortalamaya dönüşür.

-   **[3. Erişim](03-retrieval.md)**

    Bir regex yönlendirici, yoğun ve BM25 kanalları, bayrak arkasında RRF, ölçülüp
    ...
    *Kapattığı tuzak:* kNN'in "yakında hiçbir şey yok" diye bir kavramı yoktur. Beş
    yıldızlı bir tatil köyü sorusuna da sekiz parça döner — halüsinasyon tam orada başlar.

-   **[4. Cevaplar ve ajanlar](04-answers.md)**

    Atıflı cevap katmanı, yerel bir modelde biten LLM sağlayıcı zinciri ve HTTP API ile
    aynı erişimciyi paylaşan bir MCP sunucusu.
...
-   **[5. Ölçüm](05-measurement.md)**
    ...
    Negatif vakalı bir altın küme, Recall@k / MRR / abstain / false_weak ve her kararın
    bir satır olduğu bir defter.

    *Kapattığı tuzak:* tek bir model ve tek bir korpus üzerinde ölçülmüş bir eşik evrensel
...
flowchart LR
    ...
    HOOK["push webhook · yoklayıcı"] --> QUEUE["iş kuyruğu"] --> WORKING
    WORKING -->|"sha256 manifest farkı"| CHANGED["değişen dosyalar"]
    CHANGED -->|"tree-sitter parçalama<br>sil + yeniden yaz"| MILVUS[("Milvus<br>yoğun + BM25")]
```
...
flowchart LR
    ...
    ROUTE -->|hayır| DENSE["yoğun<br>(bayraklar: hybrid RRF · rerank)"] --> HIT
...
=== "Bir ajandan"
    ...
    verir; erişimcinin işi aday sunmak ve ne kadar emin olduğu konusunda dürüst olmaktır.

---

## Sayılar ne diyor

Tam tablolar, geldikleri korpus ve çekinceler **[Ölçüm](05-measurement.md)** sayfasında.
...
| Parça zenginleştirme (LLM açıklamaları) | Türkçe düz metin MRR 0.778 → **0.932**, `false_weak` yarıya indi |
| Üç bantlı çekimserlik | Negatiflerde 0.846 abstain, %4.8 yanlış alarmla; rerank kapısı %28.6'ya mal oldu |
