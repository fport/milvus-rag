...
Dört kanal, bir yönlendirici ve cevabın korpusta olmadığı durumda ne yapılacağına dair
...
!!! done "Bu aşamanın sahibi olduğu yerler"
    ...
    `config.py` içindeki üç bantlı çekimserlik eşikleri.
...
benzer görünen başka camelCase tanımlayıcılardır. BM25 onu birebir bulur, çünkü sözcüksel
...
oluyor; diğer yönde yanılmak yalnızca bir iyileştirmeye. Bu yüzden desen tek bir jeton
...
!!! measured "Neden her zaman iki kanal birleştirilmiyor"
    ...
    | **auto** (sembol→BM25, metin→yoğun) ✓ | **0.786** | **0.690** | 0.684 | 34 ms |

    Her zaman birleştirmek, tek başına yoğun kanaldan *daha kötü* (MRR 0.604'e karşı
    ...
    Yönlendirme bir regex'e mal oluyor ve yalnız yoğun kanala göre 0.012 MRR
...
normalizasyon sabiti gerektirir ve o sabit korpusla birlikte kayar: bir TypeScript
...
!!! measured "Sıralamayı kötüleştirdi"
    ...
    |---|---|---|---|---|
    | auto + yoğun | 0.786 | **0.690** | 0.684 | 34 ms |
...
korpusunda yeniden çalıştırabilsin.
...
bunları okuyor, bir hata ayıklama oturumu da yanlış bir parçanın neden başa geldiğini
açıklamak için.
...
Erişimciye bir TypeScript API'sine karşı *"hiç beş yıldızlı bir tatil köyünde bulundun
mu?"* diye sor — kendinden emin biçimde sekiz parça döndürür, en iyisi 0.366. Bunları
...
!!! measured "Gri bölgede kosinüs ayırmıyor"
    ...
    Altın küme top-1 yoğun: medyan 0.636, **min 0.526**.
    Alakasız top-1 yoğun: medyan 0.531, **maks 0.598**.
...
Taban, altın kümedeki en düşük gerçek cevabın epey altında duruyor, yani yalnızca saçma
...
!!! measured "Neden not, neden reranker kapısı değil"
    ...
    0.45 tabanı altın kümeden hiçbir şey düşürmedi ve iki negatifi ("tatil köyü" 0.366,
    "Kafka rebalance" 0.418) sıfır sonuca indirdi.

### Eşikler evrensel değil

Bunlar **bu embedding modeli ve bu korpus için** ölçüldü. Kosinüs dağılımı modelle
...
| `RAG_PROSE_MODE` | `dense` | `dense` ya da `hybrid` (yoğun + BM25 → RRF) |
